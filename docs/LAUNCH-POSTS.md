...
Scope to keep honest everywhere: the CLI **runs** GMI Studio generation (image/video/audio models, published Workflows) + LLM inference. It does **not** create/edit workflows — that's the GMI Studio visual editor. Don't imply otherwise.
...
>
> So I turned the whole pipeline into an open-source CLI for GMI Studio. Your terminal is now a film studio. It's live on npm today 🧵
...
> It's also an MCP server. One line wires it into Claude Code, Cursor, Codex, Windsurf, Cline, Kilo, Grok, or Factory:
>
...
> - **It's agent-native.** The same package is an MCP server, so Claude Code, Cursor, Codex, Windsurf, Cline, Kilo, Grok, and Factory can drive it. "Make me 5 vertical clips and download them" becomes a real workflow.
...
>
> Retries never double-submit a paid job (429 retries everywhere, 5xx only for reads). 39 tests, CI on ubuntu+macos × Node 22/24. Unofficial, MIT, bring your own key. Scope note: it runs generation (models + published Studio workflows) and LLM inference; it doesn't author workflows — that's the Studio UI.
>
...
> Hey all — I've been building a lot on GMI Studio and made an open-source CLI + MCP server to drive it from the terminal (and from Claude Code / Cursor / Codex). It's live on npm now. Sharing in case it's useful, plus a couple of things I learned the hard way that might save you credits:
>
...
>
> **The tool** (`gmi`): one command does upload → generate → poll → download, auto-converts any file format, and — the part I like most — prices every job before submitting so a bad payload fails for $0. Also an MCP server for AI IDEs.
>
> `npm install -g unofficial-gmi-cloud-cli`
> https://github.com/yannan000/unofficial-gmi-cloud-cli
>
